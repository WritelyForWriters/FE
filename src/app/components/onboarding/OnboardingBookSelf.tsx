...
import styles from './OnboardingBookSelf.module.scss'
...
function OnboardingMainHeader({ stepIndex }: { stepIndex: number }) {
  const { onClose } = useCollapsed()
    ...
    <header className={cx('header')}>
      <div className={cx('inner-container')}>
        ...
        <section className={cx('inner-container__buttons')}>
          ...
          <SelectMenu
            handleClose={onClose}
            isOpen={stepIndex === 2 || stepIndex === 3}
            style={{ width: '109px' }}
          >
            <SelectMenu.Option
              option={{
